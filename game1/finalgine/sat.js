...
finalgine.SAT.getCollision = function (A, B, offset, vel, finalDepth, finalAxis) {
    var taxis = [];
    var naxis = [];
    ...
    if (f > 0.00001) {
        ...
        if (!finalgine.SAT.intervalIntersect(A, B, offset, vel, axis, taxis, naxis)) {
            return false;
        }
    }
    ...
    // Find MTV, because we have not found a separation, therefore there must be an overlap, either in time or now
    if (finalgine.SAT.findMTV(taxis, naxis, taxis.length, finalDepth, finalAxis)) {
...
finalgine.SAT.intervalIntersect = function (A, B, offset, vel, axis, taxis, naxis) {
    // Project both bounding volumes on current axis
    var projection1 = A.project(axis);
    var projection2 = B.project(axis);

    // Offset projection
    var d = offset.dot(axis);
    projection1.min += d;
    projection1.max += d;
    ...
    if (d0 > 0 || d1 > 0) {
        ...
        var v = vel.dot(axis);
        if (Math.abs(v) < finalgine.MinFloat) {
            return false;
        }
        ...
        // Save the minimum positive overlap in time (0.0 - 1.0 = 1.0 No collision at all)
        taxis.push(overlap);
...
finalgine.SAT.findMTV = function (taxis, naxis, count, finalDepth, finalAxis) {
    var mini = -1;
    finalDepth.x = 0.0;
